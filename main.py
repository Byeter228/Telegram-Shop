...
def start_bot():
    ...
    def handler_start(message):
        ...
        bot.send_message(chat_id,
                         'Привет 👋 {}, 🔑 id - {}\nВ этом магазине ты найдёшь самый качественный товар 🔥'.format(message.from_user.first_name,
                                                                    chat_id,),
    ...
    # Qiwi Send Money [Last Step]
    def qiwi_send_money_three(message, account, amount):
    	if message.chat.id == settings.admin_id:
            try:
                if message.text == 'Отмена':
                    bot.send_message(chat_id=message.chat.id, text="Вы отменили действие...\n➖➖➖➖➖➖➖➖➖➖\nНастройки платежных систем", reply_markup=menu.pay_menu_control_catalog)
                else:
                    comment = message.text
                    api = QApi(token=settings.QIWI_TOKEN, phone=settings.QIWI_NUMBER)
                    api.pay(account=account, amount=amount, comment=comment)
                    bot.send_message(chat_id=message.chat.id, text="✅ Деньги успешно отправлены!\nЕсли деньги не пришли, значит вы неверно указали данные.\n\nНастройки платежных систем", reply_markup=menu.pay_menu_control_catalog)
            except:
                ...
                bot.send_message(chat_id=message.chat.id, text="❌ Ошибка!\n\n{}\n{}\n{}\n{}\n{}\n➖➖➖➖➖➖➖➖➖➖\nНастройки платежных систем".format(text, text_1, text_2, text_3, text_4), reply_markup=menu.pay_menu_control_catalog)

    # Qiwi Send Money [Second Step]
    def qiwi_send_money_second(message, account):
        if message.chat.id == settings.admin_id:
            if message.text == 'Отмена':
                bot.send_message(chat_id=message.chat.id, text="Вы отменили действие...\n➖➖➖➖➖➖➖➖➖➖\nНастройки платежных систем", reply_markup=menu.pay_menu_control_catalog)
            else:
                amount = message.text
                msg = bot.send_message(chat_id=message.chat.id, text="Введите примечание к переводу:")
                bot.register_next_step_handler(msg, qiwi_send_money_three, account, amount)

    # Qiwi Send Money
    def qiwi_send_money(message):
        if message.chat.id == settings.admin_id:
            if message.text == 'Отмена':
                bot.send_message(chat_id=message.chat.id, text="Вы отменили действие...\n➖➖➖➖➖➖➖➖➖➖\nНастройки платежных систем", reply_markup=menu.pay_menu_control_catalog)
            else:
                account = message.text
                msg = bot.send_message(chat_id=message.chat.id, text="Введите сумму перевода:")
                bot.register_next_step_handler(msg, qiwi_send_money_second, account)
    ...
    def handler_call(call):
        ...
        if call.data == 'catalog':
            bot.edit_message_text(
                ...
                text='Каталог',
                reply_markup=func.menu_catalog(chat_id)
            )

        if call.data == 'exit_catalog':
            bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
        ...
        if call.data in func.list_sections():
            ...
            if str(call.data) != str(func.dwn_lcd_user(chat_id)):
                    section_index = 1
                    lcd = ''
                    func.update_user(chat_id, section_index, lcd)


            func.update_user_lsd(chat_id, call.data)

            bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
                text=f'❕ Выберите нужный товар',
                reply_markup=func.menu_section(call.data, chat_id)
            )
        ...
        if call.data == 'right_catalog_list':
            catalog_index += 1
        
        if call.data == 'left_catalog_list':
            catalog_index -= 1
        
        if call.data == 'right_section_list':
            lcd = ""
            section_index = 1
            section_index = func.dwn_section_user(chat_id)
            section_index += 1
            func.update_user(chat_id, section_index, lcd)
            ...
            bot.edit_message_text(
                ...
                message_id=message_id,
                text=f'❕ Выберите нужный товар',
                reply_markup=func.menu_section("Update", chat_id)
            )

        if call.data == 'left_section_list':
            lcd = ""
            section_index = 1
            section_index = func.dwn_section_user(chat_id)
            section_index -= 1
        ...
        if call.data == 'delete_section_catalog':
            if chat_id == settings.admin_id:
                ...
                bot.register_next_step_handler(msg, delete_section)

        if call.data == 'add_product_section':
            if chat_id == settings.admin_id:
                conn = sqlite3.connect("bd.sqlite")
                cursor = conn.cursor()
                cursor.execute('SELECT * FROM catalog')
        ...
        if call.data == 'delete_product_section':
            if chat_id == settings.admin_id:
                conn = sqlite3.connect("bd.sqlite")
                cursor = conn.cursor()
                cursor.execute('SELECT * FROM catalog')
        ...
        if call.data == 'product_download':
            ...
            msg = bot.send_message(chat_id=chat_id,
                                   text='Введите номер раздела\n\n'
                                        f'{text}')
            bot.register_next_step_handler(msg, product_download)
        ...
        if call.data == 'balance_repenish':
            bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
                text='Выберите способ пополнения',
                reply_markup=menu.balance_repenish
            )
        if call.data == 'balance_renish':
            bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
                text='Что бы пополнить баланс через BTC нужно отправить \n\n' \
                     '1. Чек BTK Banker \n\n' \
                     '2. Свой ID (Его вы можете посмотреть во вкладке профиль)\n' \
                     ' \n' \
                     ' Возможна задержка, так как операция проводится в ручную.',
                                  reply_markup=menu.balance_repenish
            )
        ...
        if call.data == 'edit_pay':
            bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
                text='Настройки платежных систем',
                reply_markup=menu.pay_menu_control_catalog
            )
            
        if call.data == 'qiwi_check_pay':
            api = QApi(token=settings.QIWI_TOKEN, phone=settings.QIWI_NUMBER)
        ...
        if call.data == 'qiwi_request':
            number = settings.QIWI_NUMBER
            bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
                text="🥝Ваш номер QIWI кошелька:\n➖➖➖➖➖➖➖➖➖➖\n💳 {}\n➖➖➖➖➖➖➖➖➖➖".format(number),
                reply_markup=menu.pay_menu_control_catalog
            )

        if call.data == 'qiwi_send':
            markup = types.ReplyKeyboardMarkup(one_time_keyboard=True, resize_keyboard=True)
            markup.add('Отмена')
            msg = bot.send_message(chat_id=chat_id, text="📱Введите номер, на который будут отправлены средства:", reply_markup=markup)
            bot.register_next_step_handler(msg, qiwi_send_money)

        if call.data == 'control_of_sections':
            bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
        ...
        if call.data == 'balance_replenish':
            func.request_code_balance(chat_id)

            menu.balance_replenish = types.InlineKeyboardMarkup(row_width=3)

            menu.balance_replenish.add(types.InlineKeyboardButton(text='🌐 Перейти к оплате', url=func.get_pay_url(chat_id)))
            menu.balance_replenish.add(
                types.InlineKeyboardButton(text='🔄 Проверить', callback_data='wait_payment'),
                types.InlineKeyboardButton(text='❌ Отменить', callback_data='cancel_payment')
            )

            bot.edit_message_text(chat_id=chat_id,
                                  message_id=message_id,
                                  text=func.balance_replenish(chat_id), parse_mode='HTML',
                                  reply_markup=menu.balance_replenish)
        ...
        if call.data == 'wait_payment':
            check = func.wait_payment(chat_id)
            if check[0] == 1:
                bot.edit_message_text(chat_id=chat_id,
                                      message_id=message_id,
                ...
                try:
                    bot.send_message(chat_id=f'-100{settings.CHANNELID}',
                                     text='💰 Пополнение баланса\n'
                                          f'🔥 От - {chat_id}\n'
                                          f'🔥 Сумма - {check[1]} руб')
                except: pass
        ...
        if call.data == 'send_message_admin':
            msg = bot.send_message(chat_id,
                                   text='Введите текст рассылки')
            bot.register_next_step_handler(msg, send_message_admin)

        if call.data == 'referral_link':
            ref_code = func.check_referral_code(chat_id)
            bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
                text=f'👥 Реферальная сеть\n\n'
                     f'Ваша реферельная ссылка:\n'
                     f'https://teleg.run/{settings.bot_name}?start={ref_code}\n\n'
                     f'За все время вы заработали - {func.check_all_profit_user(chat_id)} ₽\n\n'
                     f'<i>Если человек приглашенный по вашей реферальной ссылки пополнит баланс, то вы получите {settings.referrral_percent} % от суммы его депозита</i>',
                reply_markup=menu.main_menu,
                parse_mode='html'
                )

        if call.data == 'admin_top_referral':
            bot.send_message(
                chat_id=chat_id,
                text=func.admin_top_referral(),
                parse_mode='html'
            )
    ...
    def give_money(message):
        try:
            ...
            bot.register_next_step_handler(msg, give_money_2)
        except Exception as e:
            bot.send_message(chat_id=message.chat.id,
                             text='⚠️ Что-то пошло не по плану',
                             reply_markup=menu.main_menu)

    def give_money_2(message):
        try:
            ...
            bot.register_next_step_handler(msg, give_money_3)
        except Exception as e:
            bot.send_message(chat_id=message.chat.id,
                             text='⚠️ Что-то пошло не по плану',
                             reply_markup=menu.main_menu)

    def give_money_3(message):
        try:
            balance = balance_dict[message.chat.id]
            if int(message.text) == balance.code:
                func.give_money(balance)
                bot.send_message(chat_id=message.chat.id,
    ...
    def create_section_2(message):
        try:
            if message.text == 'Yes':
                ...
                func.add_section_catalog(catalog.name)
                bot.send_message(
                    chat_id=message.chat.id,
    ...
    def delete_section(message):
        try:
            ...
            bot.register_next_step_handler(msg, delete_section_2)
        except Exception as e:
            bot.send_message(chat_id=message.chat.id,
                             text='Упсс, что-то пошло не по плану')

    def delete_section_2(message):
        try:
            if message.text == 'Yes':
                catalog = catalog_dict[message.chat.id]
                func.delete_section_catalog(catalog.name)
                bot.send_message(
                    chat_id=message.chat.id,
                    text=f'✅Раздел: {catalog.name}\n'
    ...
    def create_product_5(message):
        try:
            if message.text == 'Yes':
                ...
                func.add_product_section(product_name, product.price, product.section, product.info)
                bot.send_message(
                    chat_id=message.chat.id,
        ...
        try:
            ...
            msg = bot.send_message(chat_id=message.chat.id,
                                   text='Выберите номер товара который хотите удалить\n\n'
                                        f'{text}')
            bot.register_next_step_handler(msg, delete_product_2)
        except Exception as e:
            print(e)
            bot.send_message(chat_id=message.chat.id,
                             text='Упсс, что-то пошло не по плану')

    def delete_product_2(message):
        try:
            product = product_dict[message.chat.id]
            ...
            msg = bot.send_message(chat_id=message.chat.id,
                                   text='❕Удалить ⬇️\n'
                                        ...
                                        f'❕{product.section}  ?',
                                   reply_markup=markup)
            bot.register_next_step_handler(msg, delete_product_3)
        except Exception as e:
            print(e)
            bot.send_message(chat_id=message.chat.id,
                             text='Упсс, что-то пошло не по плану')

    def delete_product_3(message):
        try:
            if message.text == 'Yes':
                product = product_dict[message.chat.id]

                func.delete_product_section(product.product, product.section)
                bot.send_message(
                    chat_id=message.chat.id,
    ...
    def product_download(message):
        try:
            ...
            bot.register_next_step_handler(msg, product_download_2)
        except Exception as e:
            bot.send_message(chat_id=message.chat.id,
                             text='Упсс, что-то пошло не по плану')

    def product_download_2(message):
        try:
            ...
            bot.register_next_step_handler(msg, product_download_3)
        except Exception as e:
            bot.send_message(chat_id=message.chat.id,
                             text='Упсс, что-то пошло не по плану')

    def product_download_3(message):
        try:
            if message.text == 'Yes':
                msg = bot.send_message(chat_id=message.chat.id,
                ...
                bot.register_next_step_handler(msg, product_download_4)
    ...
    def send_message_admin(message):
        dict = func.send_message_admin(message.chat.id)
        send_message_admin_dict[message.chat.id] = dict

        dict = send_message_admin_dict[message.chat.id]
        dict.text = message.text

        msg = bot.send_message(message.chat.id,
                               text='Отправьте "ПОДТВЕРДИТЬ" для подтверждения')
        bot.register_next_step_handler(msg, send_message_admin_2)

    def send_message_admin_2(message):
        conn = sqlite3.connect('bd.sqlite')
        cursor = conn.cursor()
        dict = send_message_admin_dict[message.chat.id]
    ...
    def product_download_4(message):
        try:
            chat_id = message.chat.id
            ...
            func.product_download(message.document.file_name, download.name_product)
